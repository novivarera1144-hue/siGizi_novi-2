...
class LaporanGlobalController extends Controller
{
    ...
    {
        // Cache hasil kalkulasi laporan global selama 5 menit (300 detik) untuk meningkatkan kecepatan navigasi
        $data = Cache::remember('admin_laporan_global_data', 300, function () use ($supabase) {
        ...
        return Inertia::render('Admin/LaporanGlobal', [
            'monthlyTrends' => fn() => $data['monthlyTrends'],
            'topFoods' => fn() => $data['topFoods'],
            'foodCategories' => fn() => $data['foodCategories'],
            'globalStats' => fn() => $data['globalStats']
        ]);
    }
}